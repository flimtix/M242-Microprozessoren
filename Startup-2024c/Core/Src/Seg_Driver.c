...
void WritePort(int val)
{
    ...
    for (i = 0; i <= 7; i++)
    {
        WRITE_SFTCLK_0;
        if (out & 0x01)
            WRITE_SDI_Pin_1; // Daten Bitweise ausgeben
        else
            WRITE_SDI_Pin_0;
        out = out >> 1;
        WRITE_SFTCLK_1;
...
void WriteValue(int digit)
{
    ...
    case 0:
        out = val % 10; // Stelle 0
        if (actdp & SEG_Driver_DP_1)
            out_dp = SEGMENT_DP;
        break;
    case 1:
        out = (val / 10) % 10; // Stelle 1
        if (actdp & SEG_Driver_DP_2)
            out_dp = SEGMENT_DP;
        break;
    case 2:
        out = (val / 100 % 10); // Stelle 2
        if (actdp & SEG_Driver_DP_3)
            out_dp = SEGMENT_DP;
        break;
    case 3:
        out = val / 1000; // Stelle 3
        if (actdp & SEG_Driver_DP_4)
            out_dp = SEGMENT_DP;
        break;
    default:
        out = 0;
    ...
    default:
        disable = 0;
        break;
    case SEG_Driver_FLASH_FAST:
        flashcount = flashcount + 3;
    case SEG_Driver_FLASH_SLOW:
        flashcount++;
    ...
    if (actvalue <= 9999 && actvalue >= 0 && !disable)
        WritePort(SEGMENT_MAP[out] & out_dp); // LSB = 7-Segment
    else
        WritePort(0xFF & out_dp);     // Anzeige Dunkel
