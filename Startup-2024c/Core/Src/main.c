...
/* Definitions for taster */
osThreadId_t tasterHandle;
uint32_t tasterBuffer[ 128 ];
osStaticThreadDef_t tasterControlBlock;
const osThreadAttr_t taster_attributes = {
  .name = "taster",
  .cb_mem = &tasterControlBlock,
  .cb_size = sizeof(tasterControlBlock),
  .stack_mem = &tasterBuffer[0],
  .stack_size = sizeof(tasterBuffer),
...
extern void TasterTask(void *argument);
...
int main(void)
{
  ...
  /* creation of taster */
  tasterHandle = osThreadNew(TasterTask, NULL, &taster_attributes);
